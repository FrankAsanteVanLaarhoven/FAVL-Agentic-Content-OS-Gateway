...
`ALLOW_ANONYMOUS_IDENTITY` (default false) is the only switch that relaxes
this. When false — everywhere reachable by an untrusted client — a request
without verified claims is rejected outright. `DEFAULT_TENANT_ID` applies only
to that anonymous development identity and to nothing else.
"""
